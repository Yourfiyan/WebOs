...
// We are starting from b2be92c, which has no glass variables.
...
// 4. Night Mode overrides
// Strip out original body.night .window that ruins the effect completely
css = css.replace(/body\.night \.window \{\n  background-color: rgba\(6, 16, 28, 0\.82\);\n  border-color: rgba\(125, 211, 252, 0\.12\);\n\}/, `body.night .window {}`);

// 5. Terminal output background adjustment
css = css.replace(/background-color: rgba\(7, 19, 32, 0\.6\);/g, 'background-color: rgba(7, 19, 32, 0.2);');

// Clean up any remaining legacy background opacity rules over Control Center and System Bar to ensure they don't somehow linger
css = css.replace(/background-color: rgba\(11, 27, 43, 0\.62\);/g, '');
css = css.replace(/background-color: rgba\(11, 27, 43, 0\.7\);/g, '');
css = css.replace(/background-color: rgba\(11, 27, 43, 0\.55\);/g, '');
  ...
  <svg style="position: absolute; width: 0; height: 0; overflow: hidden;" aria-hidden="true">
    <defs>
      <filter id="os-lens-refraction" x="-20%" y="-20%" width="140%" height="140%">
        ...
        <!-- Map the displacement. Scale regulates strength of magnification. -->
...
// Clean any accidental remnants of the lens duplicated DOM layers from previous attempts
html = html.replace(/\s*<div class="os-glass-lens"><\/div>/g, '');
html = html.replace(/\s*<div class="os-glass-surface"><\/div>/g, '');
